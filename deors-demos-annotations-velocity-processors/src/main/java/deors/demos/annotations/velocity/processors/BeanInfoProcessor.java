...
/**
 * Annotation processor for BeanInfo annotation type. It generates a full
 * featured BeanInfo type with the help of an Apache Velocity template.
 *
 * @author deors
 * @version 1.0
 */
...
public class BeanInfoProcessor extends AbstractProcessor {
	...
	/**
	 * Reads the BeanInfo information and writes a full featured BeanInfo type with
	 * the help of an Apache Velocity template.
	 *
	 * @param annotations
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		try {
			...
			Map<String, BeanInfoPropertyModel> properties = new HashMap<String, BeanInfoPropertyModel>();
			Map<String, BeanInfoMethodModel> methods = new HashMap<String, BeanInfoMethodModel>();
			...
			for (Element e : roundEnv.getElementsAnnotatedWith(BeanInfo.class)) {
				...
				if (e.getKind() == ElementKind.CLASS) {
					...
					processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
							"annotated field: " + property.getName() + " // field type: " + property.getQualifiedType(),
							e);
					...
					List<BeanInfoPropertyModel> parameters = new ArrayList<BeanInfoPropertyModel>();
					...
					processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
							"annotated method: " + method.getName() + " // return type: " + method.getQualifiedType(),
							e);
					for (BeanInfoPropertyModel parameter : parameters) {
						processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE, "parameter: "
								+ parameter.getName() + " // parameter type: " + parameter.getQualifiedType(), e);
					}
				}
			}
			...
			if (model != null) {
				...
				JavaFileObject jfo = processingEnv.getFiler().createSourceFile(model.getBeanInfoQualifiedName());
